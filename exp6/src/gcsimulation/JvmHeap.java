...
public class JvmHeap extends MyHeap<MyObject> {
    ...
    /*@ public normal_behavior
      ...
      @         (\forall int j; 0 <= j && j < objectId.size(); objectId.get(j) != elements[i].getId())
      @         ==>  (elements[i] == \old(elements[i]))); 要求：定义后置条件，表示调用此方法前后，elements数组中下标大于等于1且小于等于size，
    ...
    public void removeUnreferenced() {
        // TODO
        Object[] oldElement = getElementData().clone();
        int oldSize = getSize();
        setSize(0);
