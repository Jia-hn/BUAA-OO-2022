...
public class Path implements Iterable<Integer> {        // 包含题目[10]-[14]
    ...
    //@ ensures \result == (\exists int i; 0 <= i && i < nodes.length; nodeId == nodes[i]);    //TODO  请使用一条\forall或\exists实现，量词只允许出现一次
    ...
    public /*@ pure @*/ boolean isValid() {
        return (size() >= 2 && getDistinctNodeCount() >= size() - 1);   //TODO  请使用类中的属性(及其方法)和运算符实现
    }
